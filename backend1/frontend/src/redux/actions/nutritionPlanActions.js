...
export const getAllPlansAction = ()=> async(dispatch) =>{
    const response = await axios.get("/api/v1/nutritionPlan/getAllPlans");
...
export const getPlanByIdAction = (id) => async (dispatch) => {
    const result = await axios.get("/api/v1/nutritionPlan/getPlan/" + id);
    console.log(result);
  ...
  export const addPlanAction = (plan) => async (dispatch) => {
    const result = await axios.post("/api/v1/nutritionPlan/createPlan", plan);
    console.log(result);
    console.log(result.data);
    dispatch({
      type: "ADD_PLAN",
  ...
  export const updatePlanAction = (plan) => async (dispatch) => {
    const result = await axios.put("/api/v1/nutritionPlan/updatePlan", plan);
    console.log(result);
    console.log(result.data);
    dispatch({
      type: "UPDATE_PLAN",
